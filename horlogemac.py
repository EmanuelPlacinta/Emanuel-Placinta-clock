...
def clear_screen():
    # Nettoyage de l'écran sur terminal macOS (séquences ANSI)
    # Fonctionne sur les terminaux POSIX comme macOS et Linux [web:18].
...
def display_time(time_tuple, is_24h):
    if is_24h:
        ...
        # Prise en compte du fait que 00h n'existe pas dans le mode AM/PM
    ...
    # Mise au format pour s'adapter à pyfiglet
    giant_time = pyfiglet.figlet_format(time_text, font='script')  # [web:26]

    # Nettoyage écran pour la lisibilité (macOS uniquement)
    clear_screen()
...
def play_alarm_sound():
    # Lecture du son via le player natif macOS afplay [web:25].
    os.system('afplay La_foule.mp3 &')

def stop_alarm_sound():
    # Arrêt du son afplay sur macOS [web:22].
    ...
    while True:
        try:
            while True:
                ...
                if alarm is not None and check_alarm(time_tuple, alarm):
                    ...
                    giant_text = pyfiglet.figlet_format(alarm_text, font='block')  # [web:26]
                    clear_screen()
                    print(giant_text)
        ...
        # Accès au menu et affichage de celui-ci
            ...
            if choice == "1":
                ...
                if alarm is None:
                    print("Il n' y avait pas d'alarme programmée.")
                    alarm = None
                else:
                ...
                # Permet d'alterner avec le précédent format d'affichage choisi
                is_24h = not is_24h
                clear_screen()
                ...
                # Pour le temps affiché (en incluant le décalage)
                ...
                bye_text = "Au revoir !"
                giant_bye = pyfiglet.figlet_format(bye_text, font='letter')  # [web:26]
                clear_screen()
                print(giant_bye)
                time.sleep(2)
                clear_screen()
